...
from sklearn.model_selection import cross_val_score, train_test_split, GridSearchCV  # 用于交叉验证和数据集划分
...
def lightgbm_cv_func(data, target, pbounds):
    ...
    optimizer = BayesianOptimization(
        f=lgb_crossval,
        pbounds=pbounds)
    # 使用GridSearchCV进行超参数调
    modelgbm=lgb.LGBMRegressor()
    grid_search = GridSearchCV(estimator=modelgbm, param_grid=pbounds, scoring='neg_mean_squared_error', cv=5)
...
# 定义一个函数，用于训练模型
def model_train(X_train, X_test, y_train, y_test, pbounds, model_type,lightgbmPbounds):
    if model_type == 'xgboost':
        ...
        best_params = lightgbm_cv_func(X_train, y_train, lightgbmPbounds)
...
def plot_prediction(date, y_true, y_pred, model_type,drug_code,drug_name,save_name=None):
    ...
    # 设置X轴标签为'Date'，Y轴标签为'Values'
    plt.xlabel('Date')
    plt.ylabel('Values')

    # 设置图形标题，包括模型类型、药物代码、R2值、MAE值、MAPE值、MSE值和药物名称
...
def model_train_and_evaluation(data, pbounds, model_types,lightgbmPbounds):
    # 分类代码列表
        ...
        if len(cate_data)<100:
            ...
            # 将数据分割为训练集和测试集，测试集大小为20%，随机种子为100以确保结果可重复
                ...
                for model_type in model_types:
                    ...
                    model_trained = model_train(X_train, X_test, y_train, y_test, pbounds, model_type,lightgbmPbounds)
                    # 将训练好的模型添加到列表中
...
if __name__ == '__main__':
    ...
    model_train_and_evaluation(data, pbounds, model_types,lightgbmPbounds)
    # p_values = range(0, 6)
    # d_values = range(0, 2)
    # q_values = range(0, 6)
